	...
	public function admin_init() {
		...
		if ( ! empty( $_REQUEST['bont-action'] ) ) {
			...
			if ( 'loginup' === $_REQUEST['bont-action'] ) {
				...
				$response = wp_remote_get( sprintf( $this->_api_login_urla, $_POST['bontact']['usernamea'], $_POST['bontact']['passworda'], $_POST['bontact']['tela'] ), array( 'sslverify' => false, 'timeout' => 30 ) );
				...
				$options = get_option( $this->_option_slug, array() );
				$options['token'] = $data_return->token;
				$options['username'] = $_POST['bontact']['usernamea'];
				$options['password'] = $_POST['bontact']['passworda'];
...
<h4>Activate Account</h4><span class="switch-but"><b>Already registered?</b> <a id="linkup" href="#" class="signin">Sign in</a></span>
		...
		<input type="hidden" name="page" value="<?php echo $this->_option_slug; ?>" />
            ...
            <div class="form-group">
				 <div class="controls">
              <input type="text" name="bontact[usernamea]" id="emaila" placeholder="Enter your email" class="form-control">
				</div>
            </div>
            <div class="form-group">
              <input type="password" name="bontact[passworda]" id="passa" placeholder="Type a password" class="form-control">
            </div>
			<div class="form-group">
              <input type="text" name="bontact[tela]" id="tela" placeholder="Enter your phone number" class="form-control">
            </div>
